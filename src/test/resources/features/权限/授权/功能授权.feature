...
Feature: 权限-功能（URL校验）
  ...
  Scenario Outline: 验证权限
    ...
    Examples:
      ...
      | 可使用结算管理,可查看告警插件,可使用入库优先级,可使用已存搜索,可使用搜索权限,可使用路由配置,可使用 Agent 管理,可查看域使用额度,可使用字典管理,可使用搜索宏,可查看用户搜索用量,可使用报表,可使用趋势图,可使用定时任务,可使用备份,可使用本地上传,可查看角色页,可查看用户页,可使用知识库,可使用仪表盘,可查看用户分组                                                                         | 403 Permission Denied | 告警插件    | 仪表盘定时任务报表趋势图知识     | 结算管理     | 入库优先级          | 搜索权限        | 403 Permission Denied | Agent 管理 | 使用限额         | 字典列表       | 搜索宏   | 搜索用量  | 报表      | 趋势图                   | 定时任务                  | 数据备份                  | 本地上传                  | 角色列表                  | 用户管理                  | 知识                    | 仪表盘                   | 用户分组                  | 403 Permission Denied | 403 Permission Denied | 403 Permission Denied | 403 Permission Denied | 403 Permission Denied | 403 Permission Denied | 403 Permission Denied | 已存搜索          | 403 Permission Denied | 403 Permission Denied | 403 Permission Denied | 路由配置  | 403 Permission Denied | 资源\n仪表盘定时任务报表趋势图知识库已存搜索搜索宏\n数据\nAgent 管理路由配置入库优先级本地上传备份策略\n权限\n角色权限用户分组用户管理搜索权限                 | 运行 (系统版本: 3.1)\n使用限额 结算管理 搜索用量           | 帮助\n联系日志易     |
