...
Feature: 权限-授权告警插件
  ...
  Scenario: 上传告警插件
   ...
   # And I "unchecked" the checkbox which name is "sendsms" in auth table
    And I "checked" the checkbox which name is "http_forwarder" in auth table
    And I "unchecked" the checkbox which name is "http_forwarder" in auth table
    And I click the "SaveButton" button
  ...
  Scenario: 验证无读取权限
    ...
    And I wait for "2000" millsecond
    Given open the "alert.PluginPage" page for uri "/plugins/"
    Then I will see the search result "{'column':'0','name':'http_forwarder','contains':'no'}"
  ...
  Scenario: 授权读取
    ...
    And I wait for loading invisible
    And I "checked" the checkbox which name is "http_forwarder" in auth table
    When I "unchecked" function "转授" from the auth table which name is "http_forwarder"
    And I click the "SaveButton" button
    And I will see the success message "更新成功"
  ...
  Scenario: 验证读取
    ...
    Given open the "alert.PluginPage" page for uri "/plugins/"
    And I wait for loading invisible
    When the data name is "http_forwarder" then i click the "授权" button
    And I wait for loading invisible
 ...
 #   And I will see the success message "更新成功"
  ...
  Scenario: 上传告警插件并勾选所有权限
    ...
    And I wait for loading invisible
    And I click the "Upload" button
    And I upload a file with name "/src/test/resources/testdata/alertPlugins/cms_syslog.py"
    And I wait for "2000" millsecond
    And I click the "EnsureButton" button
    And I wait for "2000" millsecond
    Given open the "roles.ListPage" page for uri "/account/roles/"
    ...
    Then I click the "{'TabButton':'告警插件'}" button
    And I wait for loading invisible
    And I "checked" the checkbox which name is "cms_syslog" in auth table
    And I click the "SaveButton" button
  ...
  Scenario: 验证转授权限
    ...
    Given open the "alert.PluginPage" page for uri "/plugins/"
    And I wait for loading invisible
    When the data name is "cms_syslog" then i click the "授权" button
