...
#    Then I wait for "SuccessMessage" will be visible
    Examples:
      | name  | domainName | supportFeature                                                                                                                                                                                                                        | dailyLimit | uploadLimit|excessLimit | excessBehavior | managerName | managerEmail       | managerPassword | inputAgain |
      | atest | autotest   | API,AgentConfiguration,Alert,Apps,Backup,Beneficiary,Dashboard,FieldExtract,Galaxee,IncidentAction,IndexManagement,Ingest,IngestPriority,Knowledge,MachineLearning,OfflineSearch,Pivot,Report,Schedule,Search,StatisticModel,Topology | 1          | 5           |3           | 拒绝采集输入     | atest       | atest@yottabyte.cn | Aa123@456       | Aa123@456  |
  ...
  Scenario Outline: 保存失败
    ...
    Examples:
      ...
      | test | test               | API            | 1          |2            |             |                |              |                    |                 |            | error message "超额次数限定 不能为空"|
      | test | test               | API            | 1          |             | 1           |                |              |                    |                 |            | error message "超额行为 不能为空"    |
      | test | test               | API            | 1          | 2           | 1           | 拒绝采集输入     |              |                    |                 |            | error message "管理员名称 不能为空"  |
      | test | test               | API            | 1          | 2           | 1           | 拒绝采集输入     | test         |                    |                 |            | error message "管理员邮箱 不能为空"  |
      | test | test               | API            | 1          |  2          | 1           | 拒绝采集输入     | test         | test               |                 |            | error message "邮箱地址 不合法"    |
      | test | test               | API            | 1          | 2           | 1           | 拒绝采集输入     | test         | tools@yottabyte.cn |                 |            | error message "密码需满足：8-16个字符" |
      | test | test               | API            | 1          | 2           | 1           | 拒绝采集输入     | test         | tools@yottabyte.cn | 12345678        | 12345678   | error message "密码需满足：至少包含数字和字母" |
      | test | test               | API            | 1          |  2          | 1           | 拒绝采集输入     | test         | tools@yottabyte.cn | 12345678a       |            | error message "密码不一致"  |
      | test | testtemp           | API            | 1          | 2            | 1           | 拒绝采集输入     | test         | atest@yottabyte.cn | 12345678a       | 12345678a  | error message "邮件名已存在\n错误码: FE_533"|
      | test | Test               | API            | 1          |  2           | 1           | 拒绝采集输入     | test         | tools@yottabyte.cn | 12345678a       | 12345678a  | error message "域名只支持小写字母、数字、下划线，且不能以下划线开头，长度小于10" |
      | test | auto_test          | API            | 1          |  2           | 1           | 拒绝采集输入     | test         | tools@yottabyte.cn | 12345678a       | 12345678a  | error message "域名已存在\n错误码: FE_531"                                 |
      | test | auto_testauto_test | API            | 1          |  2           | 1           | 拒绝采集输入     | test         | tools@yottabyte.cn | 12345678a       | 12345678a  | error message "域名只支持小写字母、数字、下划线，且不能以下划线开头，长度小于10" |
      | test | test               | API            | 200000001  |   2          | 1           | 拒绝采集输入     | test         | tools@yottabyte.cn | 12345678a       | 12345678a  | error message "每日限额 需小于 8388608GB"                                 |
      | test | test               | API            | 1          |  2           | 2147483648  | 拒绝采集输入     | test         | tools@yottabyte.cn | 12345678a       | 12345678a  | error message "超额次数限定 需小于 2147483647"                             |
