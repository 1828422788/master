@saveAsTrend @saveAsTrendStatistics
Feature: 保存为趋势图_普通统计视图
...
# can not compare, because of same values
  ...
  Scenario Outline: fieldnumber1(RZY-2727)
    ...
    And I drag the scroll bar to the element "Chart"
    And I wait for "2000" millsecond
    And take part of "Chart" with name "actual/保存为趋势图/普通统计视图/2727_字段数值/2727_<statType>_<chart>_<timeSpan><time>"
  ...
  Scenario Outline: fieldnumber3
    ...
    And I drag the scroll bar to the element "Chart"
    And I wait for "2000" millsecond
    And take part of "Chart" with name "actual/保存为趋势图/普通统计视图/2727_字段数值/2727_<chart>_<timeSpan>_clientip"
  ...
  Scenario Outline: totalpercent(RZY-819,2730)
    ...
    And I drag the scroll bar to the element "Chart"
    And I wait for "2000" millsecond
    And take part of "Chart" with name "actual/保存为趋势图/普通统计视图/<screenName>"
...
#    Then I compare source image "actual/保存为趋势图/普通统计视图/822_多级统计/822_chart_step3" with target image "expect/保存为趋势图/普通统计视图/822_多级统计/822_chart_step3"
  ...
  Scenario:geogrdistr(RZY-825)
    ...
    And I wait for "Chart" will be visible
    And I wait for "2000" millsecond
    And take part of "Chart" with name "actual/保存为趋势图/普通统计视图/825_地理分布/825_world"
