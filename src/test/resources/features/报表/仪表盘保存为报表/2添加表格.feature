...
Feature:2添加表格
  ...
  Scenario Outline:  add_trends
    ...
    Examples:
      ...
      | 维度     | Pie          | Pie_1               | Pie_Red_ratio0.9_seg2                                               | tag:sample04061424_chart \| stats count(apache.clientip) as ip_count by apache.clientip \| sort by ip_count \| limit 5 |
      | 维度     | Pie          | Pie_2               | Pie_Green_ratio0_seg3                                               | tag:sample04061424_chart \| stats count(apache.clientip) as ip_count by apache.clientip \| sort by ip_count \| limit 5 |
      ...
      | 维度     | Bar          | Bar3                | vertical_全部展示_Red_横向_纵向                                     | tag:sample04061424_chart \| stats count(apache.clientip) as ip_count by apache.clientip \| sort by ip_count \| limit 5 |
      | 维度     | Bar          | Bar4                | vertical_只展示名称_Orange_纵向_横向                                | tag:sample04061424_chart \| stats count(apache.clientip) as ip_count by apache.clientip \| sort by ip_count \| limit 5 |
      | 维度     | Sun          | Sun                 | DarkBlue                                                            | tag:sample04061424_chart \| stats count() by apache.status,apache.geo.province, apache.geo.city |
      | 维度     | Flame        | Flame               | Flame                                                               | tag:sample04061424_chart AND (apache.status:200) AND NOT (apache.geo.city:黔东南苗族侗族自治州) AND NOT (apache.geo.city:南京市) \| stats count() as cnt by apache.method, apache.status, apache.geo.province, apache.geo.city \| sort by apache.method, apache.status, apache.geo.province, apache.geo.city |
      ...
      | 其他     | Ring         | Ring_oneField       | Orange                                                              | tag:sample04061424_chart \| stats count(apache.clientip) as ip_count by apache.clientip \| sort by ip_count, apache.clientip \|eval count_perc=ip_count/50 \| eval count2_perc=ip_count/200 \| limit 6 |
      | 其他     | Ring         | Ring_twoFields      |                                                                     | tag:sample04061424_chart \| stats count(apache.clientip) as ip_count by apache.clientip \| sort by ip_count, apache.clientip \|eval count_perc=ip_count/50 \| eval count2_perc=ip_count/200 \| limit 6 |
