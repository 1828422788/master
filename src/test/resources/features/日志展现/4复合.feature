...
#4(3)
  ...
  Scenario Outline: compound(RZY-835)
    ...
    Examples:
      ...
      |   ts   |   count_   |     count_     |    upper   |     lower  |    Rangeline  |    835_1   | tag: autosample_chart \| bucket timestamp span=3h as ts \| stats count(appname) as count_ by ts \| movingavg count_,5 as ma \| rollingstd count_,5 as rs \| eval lower=ma-3*rs \| eval upper=ma+3*rs \| eval outlier=if(count_>upper \|\|count_<lower, count_, null)  |
      |   ts   |   cnt      |   _predict_cnt |    upper95 |    lower95 |    Rangeline  |    835_2   | tag: autosample_chart \| bucket timestamp span=1h as ts \| stats count\(\) as cnt by ts \| esma cnt timefield=ts  |
  ...
  Scenario Outline: manyy(RZY-2785,2786)
    ...
    Examples:
      ...
      |   Multiaxis   |  Today   |   2786    | tag: autosample_display \| stats count(apache.resp_len), max(apache.resp_len), min(apache.resp_len), sum(apache.status), avg(apache.resp_len) by apache.resp_len,apache.status \| limit 10|
