...
#    And I will see the element "WhitelistRoot" name is "hekad-daemon\.log"
  ...
  Scenario Outline: Agent添加数据源-单一数据源采集-utf-16
    Given the data name "autohekafiletest" in table "AppNameTable" then i click the "删除" button
    And I click the "Ensure" button
    Then I will see the element "ChangeMemo" name is "删除 Agent 配置成功。"
    And I click the "Create" button
...
#    And I set the agent parameter "IP" with running ip
  ...
  Scenario: 脚本类型数据源删除
    Given the data name "/sbin/service" in table "SyslogTable" then i click the "删除" button
    And I click the "Ensure" button
    Then I will see the element "ChangeMemo" name is "删除 Agent 配置成功。"

  Scenario Outline: Agent添加数据源-脚本采集-utf-16
#    And I set the agent parameter "IP" with running ip
    And I wait for loading invisible
    And I click the "Create" button
    And I click the "ScriptType" button
  ...
  Scenario Outline: Agent添加数据源-脚本采集-gbk
    Given the data name "/sbin/service" in table "SyslogTable" then i click the "删除" button
    And I click the "Ensure" button
    Then I will see the element "ChangeMemo" name is "删除 Agent 配置成功。"
    And I wait for loading invisible
